...
class RetrieverPipeline {
  ...
  Future<List<VectorSearchResult>> retrieve(String queryText, {int limit = 5, double minScore = 0.0}) async {
    final queryEmbedding = await manager.embeddingProvider.embedText(queryText);
    return manager.vectorStore.search(queryEmbedding, limit: limit, minScore: minScore);
  }

  Future<String> retrieveAndBuildContext(String queryText, {int limit = 3, double minScore = 0.1}) async {
    ...
    for (final res in results) {
      ...
      buffer.writeln('- **$title** (Category: $category, Relevance: $scorePct%):');
      buffer.writeln('  > $text');
      buffer.writeln();
    }
