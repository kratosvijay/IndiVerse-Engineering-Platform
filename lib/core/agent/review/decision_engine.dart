...
class DecisionEngine {
  ...
  DecisionRecord evaluate(DecisionContext context, {String? parentDecisionId}) {
    ...
    if (context.review.overallScore < 7.0 || context.confidence.overall < 0.70) {
      action = DecisionAction.replan;
      reason = 'Low code review score (${context.review.overallScore}) or low confidence (${context.confidence.overall}). Triggering replan.';
    } else if (context.hasSecurityRisk) {
      action = DecisionAction.requestReview;
      reason = 'Security flags identified. Requesting architectural code reviews.';
    } else if (context.estimatedLoc > 500) {
      action = DecisionAction.askUser;
      reason = 'Change size is large (${context.estimatedLoc} LOC). Prompting user for approval.';
    } else {
