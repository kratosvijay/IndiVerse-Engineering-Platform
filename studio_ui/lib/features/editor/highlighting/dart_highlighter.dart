...
class DartHighlighter extends SyntaxHighlighter {
  static const _keywords = {
    'class', 'import', 'void', 'final', 'const', 'return', 'async', 'await',
    'if', 'else', 'for', 'in', 'while', 'switch', 'case', 'break', 'continue',
    'late', 'var', 'get', 'set', 'extends', 'implements', 'with', 'override',
    'static', 'factory', 'dynamic', 'int', 'double', 'num', 'bool', 'String',
    'List', 'Map', 'Set', 'Future', 'Stream', 'ChangeNotifier', 'Widget', 'BuildContext'
  };
      ...
      if (line.trim().startsWith('//')) {
        ...
        for (final word in words) {
          ...
          if (startIdx > currentPos) {
            ...
            spans.add(TextSpan(text: gap, style: const TextStyle(color: SyntaxTheme.normal)));
          }
          
          TextStyle style;
          if (_keywords.contains(word)) {
            style = const TextStyle(color: SyntaxTheme.keyword, fontWeight: FontWeight.bold);
        ...
        if (currentPos < line.length) {
          spans.add(TextSpan(text: line.substring(currentPos), style: const TextStyle(color: SyntaxTheme.normal)));
        }
      }
